...
rem  NEXT TO THE EXE - that is, inside bin\. The clean step below deletes
rem  all of bin\, so data\ is moved out to TEMP first and moved back when
...
set "DATADIR=src\ServerMonitor\bin\Release\net45\data"
...
if exist "%BACKUP%" (
    if exist "%DATADIR%\*" (
        ...
        if errorlevel 8 (
            ...
            echo   WARNING: the old backup could not be put back into bin\.
...
if exist "%DATADIR%\*" (
    ...
    if errorlevel 8 (
        ...
        rem earlier files in BACKUP and the rest in bin\ - the data would be
        rem split in two. Put those back first so the program still runs, and
        rem stop before anything is deleted.
        robocopy "%BACKUP%" "%DATADIR%" /e /move /nfl /ndl /njh /njs /np /r:1 /w:1 >nul
...
if exist "%BACKUP%\*" (
    if not exist "src\ServerMonitor\bin\Release\net45" mkdir "src\ServerMonitor\bin\Release\net45"
    robocopy "%BACKUP%" "%DATADIR%" /e /move /nfl /ndl /njh /njs /np /r:1 /w:1 >nul
...
:fail_copyout
echo.
echo ERROR: cannot copy %DATADIR% out of bin\ - nothing was deleted.
